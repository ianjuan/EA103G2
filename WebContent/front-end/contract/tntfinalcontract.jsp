...
<head>
	...
	<script type="text/javascript" src="<%=request.getContextPath()%>/front-end/contract/js/tntsign.js" charset="UTF-8"></script>
	...
	<div id="body">
		...
		<form class="table" name="recForm" METHOD="post" ACTION="<%=request.getContextPath()%>/rec/RecServlet">
			<div id="center">
				...
				<div id="cbody">				
						 立契約書人承租人    ，出租人<%=tntVO.getTnt_name()%>茲為房屋租賃事宜，雙方同意本契約條款如下： <br>
						 第一條 房屋租賃標的 <br>
 						 一、房屋標示： <br>
						 (一)門牌<%=houseVO.getHos_add()%>。 <br>
						 (二)面積共計<%=houseVO.getHos_pnum()%>平方公尺。 <br>
						 (三)車位： <br>
						 1.車位種類： <br>
						 <%=houseVO.getHos_park()%>。 <br>
						 2.使用時間： <br>
						 ■全日□日間□夜間□其他___。 <br>
						 (三)租賃附屬設備： <br>
						 (四)其他：   。 <br>
						 ...
						 租金支付方式：□現金繳付■轉帳繳付 <br>
						 第四條 擔保金（押金）約定及返還 <br>
						 擔保金（押金）由租賃雙方約定為兩個月租金，金額為<%=houseVO.getHos_rentfee()*2%>元整(最高不得超過二個月房屋租金之總額)。承租人應於簽訂本契約之同時給付出租人。 <br>
						 ...
						 一、管理費： <br>
 						 □由出租人負擔。 <br>
						 ■由承租人負擔。 <br>
						 房屋每月<%=houseVO.getHos_manafee()%>元整。 <br>
						 租賃期間因不可歸責於雙方當事人之事由，致本費用增加者，承租人就增加部分之金額，以負擔百分之十為限；如本費用減少者，承租人負擔減少後之金額。 <br>
						 □其他：      。 <br>
						 二、水費： <br>
						 □由出租人負擔。 <br>
						 ■由承租人負擔。 <br>
						 ■每度<%=houseVOwaterfee.getHos_waterfee()%>元 <br>
						 □每月__元 <br>
						 □其他：______。(例如每度  元整) <br>
						 三、電費： <br>
						 □由出租人負擔。 <br>
						 ■由承租人負擔。 <br>
						 ■每度<%=houseVOelectfee.getHos_electfee()%>元 <br>
						 □每月__元 <br>
						 □其他：______。(例如每度  元整) <br>
						 四、瓦斯費： <br>
						 □由出租人負擔。 <br>
						 ■由承租人負擔。 <br>
						 ■每月<%=houseVO.getHos_gasfee()%>元 <br>
						 □其他：______。 <br>
						 五、其他費用及其支付方式： <br>
						 ■公共水費<%=houseVO.getHos_puwaterfee()%>元 <br>
						 ■公共電費<%=houseVO.getHos_puelefee()%>元 <br>
						 ■網路<%=houseVO.getHos_netfee()%>元 <br>
						 □其他：______。 <br>
						 第六條 稅費負擔之約定  <br>
