...
public class TntServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getOne_For_Display".equals(action)) {// 來自select_page.jsp的請求
			...
			try {
				...
				String regex = "^(TNT|tnt)[0-9]{6}$";
				if (!tnt_no.matches(regex)) {
					errorMsgs.add("房客編號格式不正確");
				}
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/back-end/tnt/select_page.jsp");
					failureView.forward(req, res);
					return;
				}
				/*************************** 2.開始查詢資料 *****************************************/
				TntService tntSvc = new TntService();
				TntVO tntVO = tntSvc.getOneTntProfile(tnt_no.toUpperCase());
				if (tntVO == null) {
					errorMsgs.add("查無資料");
				}
				if (!errorMsgs.isEmpty()) {
				...
				String url = "/back-end/tnt/listOneTnt.jsp";
		...
		if ("getOne_For_Update".equals(action)) { // 來自listAllTnt.jsp的請求
			List<String> errorMsgs = new LinkedList<String>();
			...
			try {
				...
				/*************************** 3.查詢完成,準備轉交(Send the Success view) ************/
				req.setAttribute("tntVO", tntVO); // 資料庫取出的tntVO物件,存入req
				String url = "/back-end/tnt/update_tnt_input.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);// 成功轉交 update_tnt_input.jsp
				successView.forward(req, res);

				/*************************** 其他可能的錯誤處理 **********************************/
			} catch (Exception e) {
				errorMsgs.add("無法取得要修改的資料:" + e.getMessage());
				RequestDispatcher failureView = req.getRequestDispatcher("/back-end/tnt/listAllTnt.jsp");
				failureView.forward(req, res);
			}

		}

		if ("update".equals(action)) { // 來自update_tnt_input.jsp的請求
			List<String> errorMsgs = new LinkedList<String>();
			// Store this set in the request scope, in case we need to
			// send the ErrorPage view.
			req.setAttribute("errorMsgs", errorMsgs);
			try {
				/*************************** 1.接收請求參數 - 輸入格式的錯誤處理 **********************/
				String tnt_no = req.getParameter("tnt_no").trim();
				...
				try {
					tnt_birth = java.sql.Date.valueOf(req.getParameter("tnt_birth").trim());
				} catch (Exception e) {
					tnt_birth = (Date) cal.getTime();
...
//				String tnt_dist = "吉安鄉";
				...
				Integer tnt_status = Integer.valueOf(req.getParameter("tnt_status"));
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req.getRequestDispatcher("/back-end/tnt/update_tnt_input.jsp");
					failureView.forward(req, res);
					return; // 程式中斷
				}
				/*************************** 2.開始修改資料 *****************************************/
				TntService tntSvc = new TntService();
//				System.out.println(tnt_no+tnt_email+tnt_acc+tnt_pwd+tnt_id+tnt_name+tnt_birth+tnt_sex+tnt_mobile+tnt_city+tnt_dist+tnt_add+tnt_pic+tnt_status);
				tntVO = tntSvc.updateTntProfile(tnt_no, tnt_email, tnt_acc, tnt_pwd, tnt_id, tnt_name, tnt_birth,
						tnt_sex, tnt_mobile, tnt_city, tnt_dist, tnt_add, tnt_pic, tnt_status);
//				System.out.println(4);
				/*************************** 3.修改完成,準備轉交(Send the Success view) *************/
				req.setAttribute("tntVO", tntVO); // 資料庫update成功後,正確的的tntVO物件,存入req
				String url = "/back-end/tnt/listOneTnt.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url); // 修改成功後,轉交listOneTnt.jsp
				successView.forward(req, res);
				/*************************** 其他可能的錯誤處理 *************************************/
			} catch (Exception e) {
		...
		if ("insert".equals(action)) { // 來自addTnt.jsp的請求
			List<String> errorMsgs = new LinkedList<String>();
			// Store this set in the request scope, in case we need to
			// send the ErrorPage view.
			req.setAttribute("errorMsgs", errorMsgs);
